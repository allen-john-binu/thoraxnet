...
class Upsamplingnew(nn.Module):

    def __init__(self,in_channels,out_channels,p1_channels,p2_channels): #can try out bilinear upsampling too instead
        super().__init__()
        ...
        self.downsize2 = nn.Sequential(nn.MaxPool3d(kernel_size=2, stride=2, padding=1),
                                       nn.Conv3d( p1_channels, p1_channels, kernel_size = 1, padding=1))
        self.downsize4 = nn.Sequential(nn.MaxPool3d(kernel_size=4, stride=4, padding=1),
                                       nn.Conv3d( p2_channels, p2_channels, kernel_size = 1, padding=1))
        self.conv3d = nn.Sequential(
            ResnetBlock(out_channels*2+p1_channels+p2_channels,out_channels),
            #ResnetBlock(out_channels,out_channels)
        )
    def shapeadjust(self,g3,x1):
    ...
    def forward(self,x1,x2,p1,p2):
        g3 = self.att(x2, x1)
        x1 = self.upsampled(x1)
        # input is CHW
        
        x1=self.shapeadjust(g3,x1)
        x = t.cat([g3, x1], dim=1)
        p1= self.downsize2(p1)
        p2= self.downsize4(p2)
        
        p2= self.shapeadjust(p1,p2)
        p = t.cat([p1, p2], dim=1)
        p=self.shapeadjust(x,p)
        x = t.cat([x, p], dim=1)
...
class UnetModel(nn.Module):
    def __init__(self,in_channels,out_classes):
        ...
        '''
        self.cv1 = ResnetBlock(in_channels,64).to(device1)
        self.down1 = Downsampling(64,128).to(device1)
        self.down2 = Downsampling(128,256).to(device1)
        self.down3 = Downsampling(256,512).to(device1)
        
        self.up1 = Upsamplingnew(512,256,128,64).to(device1)
        #self.up1 = Upsampling(512,256).to(device1)
        self.up2 = Upsampling(256,128).to(device1)
        self.up3 = Upsampling(128,64).to(device2)
        self.cv2 = OutputConv(64,out_classes).to(device1)
        '''
        self.cv1 = ResnetBlock(in_channels,c).to(device1)
        self.down1 = Downsampling(c,c*2).to(device1)
        self.down2 = Downsampling(c*2,c*4).to(device1)
        self.down3 = Downsampling(c*4,c*8).to(device1)
        
        self.up1 = Upsamplingnew(c*8,c*4,c*2,c).to(device1)
        #self.up1 = Upsampling(512,256).to(device1)
        self.up2 = Upsampling(c*4,c*2).to(device1)
        self.up3 = Upsampling(c*2,c).to(device2)
        self.cv2 = OutputConv(c,out_classes).to(device1)
        self.initialize()
    ...
    def forward(self,x):
        ...
        x= self.up1(x4.to(device1),x3.to(device1),x2.to(device1),x1.to(device1))
